  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-border bg-card p-4">
        <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-1">{t('compliancePanel.allowedLimit')}</p>
      ...
      <div className={cn(
        ...
        <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-1">{t('compliancePanel.recommendedEmission')}</p>
        <motion.p
          key={currentEmission}
          initial={{ scale: 1.03 }}
          animate={{ scale: 1 }}
        ...
        {!isCompliant && (
          <p className="text-xs text-destructive/80 mt-1">
            {t('compliancePanel.exceedsBy')} {remaining.toFixed(1)} {unit}
          </p>
        )}
      </div>
      ...
      <div className={cn(
        ...
        <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-1">{t('compliancePanel.remainingReduction')}</p>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">{t('compliancePanel.complianceProgress')}</p>
          <span className="text-xs font-bold tabular-nums text-muted-foreground">{Math.round(progress)}% {t('compliancePanel.reductionAchieved')}</span>
        </div>
      ...
      >
        <div className="flex items-center justify-center gap-2">
          ...
          <span className={cn("text-lg font-bold", isCompliant ? "text-emerald-600 dark:text-emerald-400" : "text-destructive")}>
            {isCompliant ? t('compliancePanel.conform') : t('compliancePanel.notConform')}
          </span>
        </div>
      </motion.div>
      ...
      {beforeValue !== undefined && (
        <div className="rounded-xl border border-border bg-card p-4">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-2">{t('compliancePanel.summary')}</p>
