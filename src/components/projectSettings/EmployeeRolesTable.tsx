            ...
            <TableBody>
              {Object.entries(groupedRoles).map(([companyName, companyRoles]) => (
                companyRoles.map((role, index) => (
                  <TableRow key={role.id} className="cursor-pointer transition-all duration-200 hover:backdrop-blur-md hover:shadow-lg hover:shadow-foreground/5 hover:scale-[1.02] hover:z-10" onDoubleClick={() => handleEdit(role)}>
