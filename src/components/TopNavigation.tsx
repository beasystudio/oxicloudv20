...
export const TopNavigation = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('storage', checkDemoData);
    };
  }, []);
  ...
  // Check for logo from settings company list as well
  ...
  const teamMembers = isPowerUser && selectedCompanyId ?
  ...
  const hasFinancialAccess = () => {
    ...
    // For admin role (e.g., Christine), check staff settings from localStorage
    if (currentUser.role === 'admin') {
      ...
      if (stored) {
        ...
        const staffMember = staffSettings.find((s: {
          email: string;
        }) => s.email === currentUser.email);
        if (staffMember) {
          return staffMember.financialDashboardAccess ?? false;
        }
      }
      // Default to false for admin if not found in settings
      return false;
    }
    // client_owner always has access
    if (currentUser.role === 'client_owner') return true;
    // For client_admin/client_user: check the user record's financialDashboardAccess setting
    const userRecord = getUserByEmail(currentUser.email);
  ...
  const renderBranding = () => {
    ...
    // Client branding: show logo OR company name, never both
    // When logo exists: show logo as the clickable trigger (with company switcher if multi-company)
    // When no logo: show company name as usual
    if (effectiveLogo) {
      if (userCompanies.length > 1 && isClientRole) {
        return <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="min-w-[180px] bg-popover border border-border shadow-lg z-50">
              {userCompanies.map((company) => <DropdownMenuItem key={company.id} onClick={() => handleSelectCompany(company.id, company.name)} className={cn("cursor-pointer", selectedCompanyId === company.id && "bg-primary/10 text-foreground")}>
                  {company.name}
                </DropdownMenuItem>)}
            </DropdownMenuContent>
          </DropdownMenu>;
    ...
    // No logo: show company name only
    return <div className="flex items-center gap-3">
                ...
                className={cn(
                  ...
                  !notification.read && "bg-primary/5"
                    ...
                    <Bell className="h-3.5 w-3.5" />
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] text-muted-foreground/60 mt-1">
                            {new Date(notification.createdAt).toLocaleString('nl-BE', {
                        day: 'numeric',
                        month: 'short',
                        hour: '2-digit',
                        minute: '2-digit'
                    ...
                    onClick={(e) => {
                      ...
                      setUnreadCount(getUnreadCount());
                      toast({
                        title: t('dashboard.nav.notificationDeleted'),
                        description: t('dashboard.nav.notificationDeletedDesc')
                      });
                    }}>
          ...
          <>
                ...
                >
                  ...
                  <DropdownMenuItem
                    ...
                    className={cn(
                      "cursor-pointer px-3 py-2.5 rounded-lg",
