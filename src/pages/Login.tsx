...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (authError) {
      ...
      toast({ title: "Welcome back!", description: "Login successful" });
      const { data: profile } = await supabase.from('profiles').select('workspace_id').eq('id', data.session.user.id).single();
      navigate(profile?.workspace_id ? '/dashboard/partner' : '/dashboard/demo');
    }
    setLoading(false);
  };
        ...
        <div className="w-full lg:w-1/2 flex items-center justify-center px-8 lg:px-12 xl:px-16 overflow-y-auto">
          <motion.div variants={container} initial="hidden" animate="show" className="max-w-[400px] w-full py-12">
            ...
            <motion.div variants={item} className="mb-10">
              <h1 className="text-3xl font-semibold text-foreground tracking-tight mb-2">{t('login.title')}</h1>
            ...
            <motion.form variants={item} onSubmit={handleSubmit} className="space-y-5 py-0 my-0">
              <div className="space-y-2">
                <Label htmlFor="email" className="text-muted-foreground text-xs uppercase tracking-wider">{t('login.emailLabel')}</Label>
                <Input
              ...
              <div className="space-y-2">
                <Label htmlFor="password" className="text-muted-foreground text-xs uppercase tracking-wider">{t('login.passwordLabel')}</Label>
                <Input
                    ...
                    <div className="space-y-1.5 pt-3">
                      {demoAccounts.map((account) => (
                        <button
                          ...
                          className="w-full group flex items-center gap-3 px-3 py-2.5 rounded-xl border border-border bg-card hover:bg-muted/50 hover:border-primary/30 transition-all duration-300">
                          <div className="w-9 h-9 rounded-full overflow-hidden bg-muted group-hover:ring-2 group-hover:ring-primary/40 transition-all duration-300 shrink-0">
                            {getAvatarByEmail(account.email) ? (
                              ...
                              <div className="w-full h-full flex items-center justify-center">
                                <span className="text-xs font-semibold text-muted-foreground group-hover:text-primary transition-colors duration-300">{account.initials}</span>
                              </div>
                            )}
                          </div>
                          <div className="flex-1 text-left min-w-0">
                            <p className="text-sm font-medium text-foreground/80 group-hover:text-foreground transition-colors truncate">{account.role}</p>
                            <p className="text-[11px] text-muted-foreground truncate">{account.email}</p>
                          </div>
                          <span className="text-[10px] text-muted-foreground bg-muted px-2 py-0.5 rounded-full shrink-0">{account.label}</span>
                        </button>
                      ))}
                    </div>
                  </motion.div>
            ...
            <motion.div variants={item} className="mt-6">
              <button onClick={() => navigate('/register')}
                className="group w-full flex items-center gap-4 px-5 py-4 rounded-xl border border-border bg-card hover:border-primary/30 hover:bg-primary/5 transition-all duration-300">
                <div className="flex-1 text-left min-w-0">
                  <p className="text-sm font-medium text-foreground/80 group-hover:text-foreground transition-colors">New to OxiCloud?</p>
                  <p className="text-xs text-muted-foreground mt-0.5 whitespace-nowrap">Create your free account</p>
                </div>
                <div className="w-8 h-8 rounded-lg bg-muted group-hover:bg-primary group-hover:rotate-45 flex items-center justify-center transition-all duration-300 shrink-0">
                  <ArrowUpRight size={14} className="text-muted-foreground group-hover:text-primary-foreground transition-colors" />
                </div>
              </button>
            </motion.div>
            ...
            <motion.div variants={item} className="mt-8 flex flex-col gap-3">
              <div className="flex items-center justify-center">
                <Link to="/forgot-password" className="text-xs text-muted-foreground hover:text-primary transition-colors">{t('login.forgotPassword')}</Link>
              </div>
