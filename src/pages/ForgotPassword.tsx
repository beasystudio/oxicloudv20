        ...
        <div className="w-full lg:w-[45%] flex flex-col justify-center px-8 lg:px-16 xl:px-20">
          <motion.div variants={container} initial="hidden" animate="show" className="max-w-sm mx-auto w-full">
            ...
            <motion.div variants={item} className="mb-10">
              <Link to="/login" className="inline-flex items-center gap-1.5 text-white/40 text-xs hover:text-primary transition-colors mb-6">
                <ArrowLeft className="h-3.5 w-3.5" />
                {t('forgotPassword.backToLogin')}
              </Link>
              <h1 className="text-3xl font-semibold text-white tracking-tight mb-2">{t('forgotPassword.title')}</h1>
              <p className="text-white/40 text-sm">{t('forgotPassword.subtitle')}</p>
            </motion.div>
            ...
            {sent ? (
              <motion.div variants={item} className="space-y-6">
                <div className="p-6 rounded-xl border border-primary/20 bg-primary/5">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-white mb-1">{t('forgotPassword.linkSentTitle')}</p>
                      <p className="text-xs text-white/50">
                        {t('forgotPassword.linkSentDesc1')} <span className="text-white/70">{email}</span> {t('forgotPassword.linkSentDesc2')}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <motion.form variants={item} onSubmit={handleSubmit} className="space-y-5">
                <div className="space-y-2">
                  <Label htmlFor="email" className="text-white/60 text-xs uppercase tracking-wider">{t('forgotPassword.emailLabel')}</Label>
                  <Input id="email" type="email" placeholder="you@company.be" value={email} onChange={e => setEmail(e.target.value)} required maxLength={255}
