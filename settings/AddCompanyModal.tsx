...
export function AddCompanyModal({
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.companyEmail.trim()) { toast({ title: t('companyModal.error'), description: t('companyModal.companyEmailRequired'), variant: "destructive" }); return false; }
    if (formData.companyEmail.trim() && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.companyEmail)) { toast({ title: t('companyModal.error'), description: t('companyModal.companyEmailInvalid'), variant: "destructive" }); return false; }
    return true;
  };
            ...
            <div className="p-6 space-y-5">
              ...
              <section className="space-y-3">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-2">
                      <Label htmlFor="companyEmail" className="text-xs">{t('companyModal.companyEmail')} *</Label>
                      <Input id="companyEmail" type="email" value={formData.companyEmail} onChange={e => setFormData(prev => ({ ...prev, companyEmail: e.target.value }))} placeholder={t('companyModal.companyEmailPlaceholder')} required />
                    </div>
                    <div className="space-y-2">
